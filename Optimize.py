...
def dichotomie(x_start, f, lim=1e-10):
    '''
    Search for f(x) = 0
...
def nelder_mead(x_start, f, max_iter = 1000, lim = 10e-6):
    '''
...
def nelder_mead_global(dimension, f, upperbound = 10, lowerbound = -10):
    res = []
    for i in range(dimension+1):
        x_start = np.random.uniform(lowerbound,upperbound, (dimension +1,dimension))
        res.append(nelder_mead(x_start, f)[0])
    x_start = []
    #print(res)
    for i in range(dimension+1):
        x_start.append(res[i][0])
    final_res = nelder_mead(x_start,f)
    return final_res[0]
